#!/bin/bash
# PM2 Manual Reload Script for Blush Marketing
# Manually trigger a zero-downtime reload
...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}Blush Marketing - Manual Reload${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""
...
# Safety check for active jobs
if [ "${SAFE_RESTART:-true}" = "true" ]; then
  echo -e "${BLUE}Checking system status...${NC}"
  echo ""
  echo -e "${YELLOW}⚠️  Reload will restart the process${NC}"
  echo -e "${YELLOW}⚠️  Long-running jobs may be interrupted${NC}"
  echo ""
  read -p "Continue with reload? (y/N) " -n 1 -r
  echo
  if [[ ! $REPLY =~ ^[Yy]$ ]]; then
    echo -e "${YELLOW}Reload cancelled${NC}"
    exit 0
  fi
fi

# Perform reload (zero-downtime for cluster mode, restart for fork mode)
echo -e "${BLUE}Reloading services...${NC}"
run_pm2 reload blush-marketing-backend 2>/dev/null || run_pm2 restart blush-marketing-backend
...
echo ""
echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}✓ Services reloaded successfully${NC}"
echo -e "${GREEN}========================================${NC}"
echo ""
