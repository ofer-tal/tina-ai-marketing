/**
 * Integration Tests for AI Chat Workflows (Live Server)
 *
 * Feature #189: Integration tests for AI chat functionality
 *
 * These tests run against the live development server
...
describe('Feature #189: Integration Tests for AI Chat Workflows (Live Server)', () => {

  /**
   * Step 1: Mock GLM4.7 API responses
   * Note: In live mode, we're testing the actual mock implementation in chat.js
   */
  describe('Step 1: GLM4.7 API Response Testing (Mock Mode)', () => {
    it('should return mock response for revenue queries', async () => {
      ...
      expect(response.body.response.content).toMatch(/revenue|mrr/i);
    });
    ...
    it('should return mock response for content strategy queries', async () => {
      ...
      expect(response.body.success).toBe(true);
      expect(response.body.response.content).toBeDefined();
      expect(response.body.response.content).toMatch(/content|post|views/i);
    });
    ...
    it('should return mock response for ads/budget queries', async () => {
      ...
      expect(response.body.success).toBe(true);
      expect(response.body.response.content).toBeDefined();
      expect(response.body.response.content).toMatch(/campaign|spend|roi|ad/i);
    });
    ...
    it('should return mock response with historical data context', async () => {
      ...
      expect(response.body.response.content).toMatch(/\$425|\$379|\$285/);
    });
